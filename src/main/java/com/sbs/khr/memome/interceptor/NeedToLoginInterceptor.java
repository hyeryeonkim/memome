...
public class NeedToLoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (isLogined == false) {
			/*
			 * if (isAjax == false) { response.setContentType("text/html; charset=UTF-8");
			 * response.getWriter().append("<script>");
			 * response.getWriter().append("alert('로그인 후 이용해주세요.');");
			 * response.getWriter().append(
			 * "location.replace('/usr/member/login?redirectUri=" +
			 * request.getAttribute("encodedAfterLoginUri") + "');");
			 * response.getWriter().append("</script>"); // 리턴 false;를 이후에 실행될 인터셉터와 액션이
			 * 실행되지 않음 } else {
			 */
				response.setContentType("application/json; charset=UTF-8");
				response.getWriter().append("{\"resultCode\":\"F-A\",\"msg\":\"로그인 후 이용해주세요.\"}");
				//request.setAttribute("redirectUti", "/usr/member/login");
